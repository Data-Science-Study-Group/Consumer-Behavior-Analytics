...
ggplot(df1, aes(x=customer_id, y=price)) + geom_point() + geom_smooth()
```
...
View(group_by_city_df)
names(group_by_city_df)

group_by_city_df %>%
group_by(customer_city) -> df_group_by_city

df_group_by_city %>% 
  arrange($) %>%
    select(customer_city_code_prefix,everything()) -> customer_city_price_plot

#fix
#ggplot(customer_zip_price_plot, aes(x=customer_zip_price_plot$customer_id,y=customer_zip_price_plot$price, ))
